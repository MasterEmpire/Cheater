    ...
    private fun executeUpload(context: Context, file: File, mimeType: String) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                notifyVoice(context, "Upload failed for image $currentInBatch. Retrying next.")
                processNext(context)
            }
        ...
        val pollRunnable = object : Runnable {
            ...
            override fun run() {
                ...
                if (attempts > 60) { // 5 minutes max
                    notifyVoice(context, "Solving timed out for image $currentInBatch")
                    processNext(context)
                    return
                }
