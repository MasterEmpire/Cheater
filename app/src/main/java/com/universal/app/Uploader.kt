...
object Uploader {
    ...
    private fun executeBatchUpload(context: Context, files: List<File>) {
        ...
        if (uploadedPaths.isNotEmpty()) {
            ...
            notifyPrompt(context, if (!isOnline(context)) 20 else 21)
            isProcessing = false
            watchdogHandler.removeCallbacks(watchdogRunnable)
        }
    }
    ...
    private fun triggerFunction(context: Context, paths: List<String>) {
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                ...
                notifyPrompt(context, if (!isOnline(context)) 17 else 28)
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                ...
                if (response.isSuccessful) {
                    ...
                    if (code == 422 || bodyStr.contains("low_quality")) {
                        notifyPrompt(context, 23)
                    } else {
                        notifyPrompt(context, 24)
                    }
                }
                response.close()
            }
    ...
    private fun notifyPrompt(context: Context, promptNumber: Int) {
        context.startService(Intent(context, PlaybackService::class.java).apply {
            action = "PLAY_PROMPT"
