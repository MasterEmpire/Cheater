...
class PlaybackService : Service(), TextToSpeech.OnInitListener {
    ...
    private var rawPromptPlayer: MediaPlayer? = null
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        when (action) {
            ...
            "SPEAK_STATUS", "PLAY_PROMPT" -> {
                val promptNum = intent.getIntExtra("prompt_num", -1)
                if (promptNum != -1) {
                    playSystemPrompt(promptNum)
                } else {
    ...
    fun playSystemPrompt(promptNumber: Int) {
        val soundResId = AmharicAudioPrompts.promptMap[promptNumber] ?: return
        val prefs = getSharedPreferences("monitor_prefs", Context.MODE_PRIVATE)
        val am = getSystemService(Context.AUDIO_SERVICE) as android.media.AudioManager
        if (prefs.getBoolean("headset_only", true) && !am.isWiredHeadsetOn && !am.isBluetoothA2dpOn) {
            DebugLogger.log("STEALTH", "Amharic prompt #$promptNumber blocked: Stealth mode active.")
            return
        }

        handler.post {
            try {
                rawPromptPlayer?.stop()
                rawPromptPlayer?.release()
                
                rawPromptPlayer = MediaPlayer.create(this, soundResId)?.apply {
                    setAudioAttributes(
                    ...
                    setOnCompletionListener {
                        it.release()
                        rawPromptPlayer = null
                    }
                    start()
                }
                DebugLogger.log("PROMPT", "Playing Amharic prompt #$promptNumber")
            } catch (e: Exception) {
                DebugLogger.log("PROMPT_ERR", "Failed to play prompt #$promptNumber: ${e.message}")
            }
        }
    }
